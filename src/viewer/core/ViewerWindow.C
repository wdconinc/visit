...
debug5 << "GetWindowAttributes: size=" << size[0] << ", " << size[1] << endl;
    ...
    winAtts.SetImageRepeatY(annot->GetImageRepeatY());
    winAtts.SetBackgroundMode((int)annot->GetBackgroundMode());
...
void ViewerWindow::SetDepthCompositeThreads(int val)
{
    visWindow->SetDepthCompositeThreads(val);
}

int ViewerWindow::GetDepthCompositeThreads() const
{
...
void ViewerWindow::SetAlphaCompositeThreads(int val)
{
    visWindow->SetAlphaCompositeThreads(val);
}

int ViewerWindow::GetAlphaCompositeThreads() const
{
...
void ViewerWindow::SetDepthCompositeBlocking(int val)
{
    visWindow->SetDepthCompositeBlocking(val);
}

int ViewerWindow::GetDepthCompositeBlocking() const
{
...
void ViewerWindow::SetAlphaCompositeBlocking(int val)
{
    visWindow->SetAlphaCompositeBlocking(val);
}

int ViewerWindow::GetAlphaCompositeBlocking() const
{
    ...
    bool detailed)
{
    ...
    if(detailed)
    {
        ...
        windowNode->AddNode(new DataNode("depthCompositeBlocking", GetDepthCompositeBlocking()));
        windowNode->AddNode(new DataNode("alphaCompositeThreads", GetAlphaCompositeThreads()));
        windowNode->AddNode(new DataNode("alphaCompositeBlocking", GetAlphaCompositeBlocking()));
        ...
        windowNode->AddNode(new DataNode("compressionActivationMode", compressionActivationMode));
    ...
    const std::string &configVersion)
{
    ...
    if((node = windowNode->GetNode("depthCompositeBlocking")) != 0)
        SetDepthCompositeBlocking(node->AsInt());
    if((node = windowNode->GetNode("alphaCompositeThreads")) != 0)
        SetAlphaCompositeThreads(node->AsInt());
    if((node = windowNode->GetNode("alphaCompositeBlocking")) != 0)
    ...
    //
    // The shading parameters are all set as a group.
    //
    numParamsSaved = 0;
    ...
    //
    // The depth parameters are all set as a group.
    //
    numParamsSaved = 0;
    ...
    if((node = windowNode->GetNode("compressionActivationMode")) != 0)
        compressionActivationMode = node->AsInt();
    ...
    if((node = windowNode->GetNode("osprayAO")) != 0)
